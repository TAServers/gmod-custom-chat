...
function SChat:AppendAtCaret(text)
	...
	local newText = oldText:sub(1, caretPos)
		.. text
		.. oldText:sub(caretPos + 1)
...
function SChat:SuggestServerTheme()
	...
	if
		Theme:GetHashFromJSON(Theme:ToJSON())
		== Theme:GetHashFromJSON(self.serverTheme)
	then
	...
	self.btnSuggest.DoClick = function()
		Derma_Query(
			"This server has a custom theme.\nDo you want to try it out?",
			"Server Theme",
			"Yes",
			function()
				local success, errMsg = Theme:Import(self.serverTheme)
				if success then
					...
					Derma_Message(
						"Error: " .. errMsg,
						"Failed to apply the server theme",
						"OK"
					)
				end
			end,
...
function SChat:OpenContextMenu(data, isLink, steamId64)
	...
	if steamId64 and steamId64 ~= "" then
		optionsMenu
			...
			:SetIcon("icon16/comment_edit.png")

		optionsMenu
			:AddOption("Copy SteamID64..", function()
				SetClipboardText(steamId64)
			end)
			:SetIcon("icon16/comment_edit.png")

		optionsMenu
			:AddOption("Open profile page..", function()
				gui.OpenURL("https://steamcommunity.com/profiles/" .. steamId64)
			end)
			:SetIcon("icon16/user.png")
	elseif data ~= "" then
		optionsMenu
			:AddOption(isLink and "Copy Link.." or "Copy...", function()
				SetClipboardText(data)
			end)
			:SetIcon("icon16/comment_edit.png")
	end
	...
	optionsMenu
		:AddOption("Find...", function()
			self.chatBox:FindText()
		end)
		:SetIcon("icon16/zoom.png")

	optionsMenu
		:AddOption("Clear everything", function()
			self.chatBox:ClearEverything()
		end)
		:SetIcon("icon16/cancel.png")
	...
	optionsMenu
		:AddOption(
			Settings.timestamps and "Disable timestamps" or "Enable timestamps",
			function()
		...
		:SetIcon(
			Settings.timestamps and "icon16/time_delete.png"
				or "icon16/time_add.png"
		)

	if Settings.allow_any_url then
		optionsMenu
			:AddOption("Block images from unknown URLs", function()
				Settings:SetWhitelistEnabled(true)
			end)
			:SetIcon("icon16/picture_delete.png")
	else
		optionsMenu
			:AddOption("Allow images from unknown URLs", function()
				Derma_Query(
					[[This option will allow images to be loaded from any URL.
This means that no matter which site they come from, you will load it, and it CAN be used to grab your IP address.]],
					"Allow unknown image URLs",
					"Allow anyway",
					function()
			...
			:SetIcon("icon16/picture_add.png")
	end

	optionsMenu:AddSpacer()

	optionsMenu
		:AddOption("Customize...", function()
			if self.usingServerTheme then
				Derma_Query(
					'You"re using a server theme. What to do want to do with it?',
					"Customize Theme",
					"Customize it (Your old theme will be lost)",
					function()
						Theme:Save()
						Theme:ShowCustomizePanel()
						self.usingServerTheme = false
					end,
					"Reset back to my old theme",
					function()
						Theme:Load()
						self:ApplyTheme(true)
		...
		:SetIcon("icon16/picture_edit.png")
		...
		optionsMenu
			:AddOption("[Admin] Chat Tags...", function()
				if self.USE_TAGS then
					...
					Derma_Message(
						[[To avoid conflicts with other addons, chat tags are disabled.
Make sure aTags is not installed.]],
						"Chat Tags",
						"OK"
					)
				end
			end)
			:SetIcon("icon16/tag_blue_edit.png")
	end
...
function SChat:SetInputEnabled(enable)
	for _, pnl in ipairs(self.frame:GetChildren()) do
		if
			pnl ~= self.chatBox
			and pnl ~= self.frame.btnMaxim
			and pnl ~= self.frame.btnClose
			and pnl ~= self.frame.btnMinim
		then
...
local function schat_ChatText(_, _, text, textType)
	...
	local canShowJoinLeave = not (
		SChat.Tags.connection.showConnect
		or SChat.Tags.connection.showDisconnect
	)
	if not canShowJoinLeave and textType == "joinleave" then
		return
	end
...
local function schat_PlayerBindPress(_, bind, pressed)
	...
	if existingBindHooks["sf_keyboard_blockinput"] then
		return
	end
...
cvars.AddChangeCallback("cl_drawhud", function(_, _, new)
	if SChat.chatBox and new == "0" then
