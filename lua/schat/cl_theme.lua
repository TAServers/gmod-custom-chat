...
function Theme:ShowCustomizePanel()
		...
		if f.index == "font" then
			...
			cmbFonts.OnSelect = function(_, index)
				if index > 1 then
					...
					Derma_StringRequest(
						"Custom Font",
						"Please enter the desired font name.",
						self.fontName,
						function(txt)
							self.fontName =
								string.JavascriptSafe(string.Trim(txt))
							SChat:ApplyTheme()
						end
					)
				end
			...
			tglFontShadow:SetIcon(
				self.fontShadow and "icon16/accept.png" or "icon16/cross.png"
			)
			...
			tglFontShadow.DoClick = function()
				...
				tglFontShadow:SetIcon(
					self.fontShadow and "icon16/accept.png"
						or "icon16/cross.png"
				)
				SChat:ApplyTheme()
			end
			...
			tglSlideAnim:SetIcon(
				self.slideAnimation and "icon16/accept.png"
					or "icon16/cross.png"
			)
	...
	btnReset.DoClick = function()
		Derma_Query(
			"Reset theme to default? All changes will be lost!",
			"Reset Theme",
			"Yes",
			function()
	...
	if not SChat:CanSetServerTheme(LocalPlayer()) then
		return
	end
	...
	btnSetServerTheme.DoClick = function()
		...
		if SChat.serverTheme == "" then
			query =
				[[This action will make all players (including those who join later) use this theme,
but only if they aren't using one already. Are you sure?]]
			...
			query =
				"This server already has a theme. What do you want to do with it?"
			actions = {
				"Override",
				function()
					setServerTheme(self:ToJSON())
...
function Theme:ShowExportPanel()
	...
	lblExportHelp:SetText(
		"Use this code to share your current theme with others!"
	)
