...
if BRANCH == "unknown" then
	SChat.PrintF(
		"Not using Chromium, enforcing plain HTTP for some image links."
	)
...
function SChat:GenerateMessageFromTable(contents)
	...
	local lines = {
		("var %s = document.createElement('div');"):format(
			JSBuilder.rootElement
		),
	}
	...
	if self.Settings.timestamps then
		...
		lines[#lines + 1] =
			code:format(JSBuilder.rootElement, os.date("%H:%M:%S"))
	end
	...
	local showTemp = (GetConVar("cl_drawhud"):GetInt() == 0) and "false"
		or "true"
	local showAnim = self.Theme.slideAnimation and "true" or "false"

	lines[#lines + 1] = ("appendMessage(%s, %s, %s);"):format(
		JSBuilder.rootElement,
		showTemp,
		showAnim
	)
...
local IsStringValid = function(s)
	return s and s ~= ""
end
local ColorToJs = function(c)
	return string.format("rgb(%d,%d,%d)", c.r, c.g, c.b)
end
local AddLine = function(t, line, ...)
	t[#t + 1] = string.format(line, ...)
end
...
JSBuilder.builders["player"] = function(val, color, font)
	...
	if not val.isBot then
		lines[#lines + 1] = JSBuilder:CreateImage(
			JSBuilder:FetchUserAvatarURL(val.id64),
			nil,
			"avatar ply-" .. val.id64
		)
	end
...
JSBuilder.builders["model"] = function(val, color, font)
	local js = ""
	local prevPath = "materials/spawnicons/"
		.. string.Replace(val, ".mdl", ".png")

	if file.Exists(prevPath, "GAME") then
		js = JSBuilder:CreateImage(
			"asset://garrysmod/" .. prevPath,
			nil,
			"emoji"
		)
	end
...
function JSBuilder:CreateText(text, font, link, color, bgColor, cssClass)
	...
	if IsStringValid(link) then
		AddLine(
			lines,
			"elText.onclick = function(){ SChatBox.OnClickLink('%s') };",
			SafeString(link)
		)
	...
	if bgColor then
		AddLine(
			lines,
			"elText.style.backgroundColor = '%s';",
			ColorToJs(bgColor)
		)
	end
...
function JSBuilder:CreateImage(url, link, cssClass, altText, safeFilter)
	...
	if safeFilter then
		...
		AddLine(
			lines,
			"elHint.onclick = function(){ removeByClass('%s'); };",
			safeguardId
		)
	else
	...
	if link then
		...
		AddLine(
			lines,
			"elImg.onclick = function(){ SChatBox.OnClickLink('%s') };",
			link
		)
		AddLine(
			lines,
			"elImg.onmouseenter = function(){ SChatBox.OnImageHover('%s', true) };",
			link
		)
		AddLine(
			lines,
			"elImg.onmouseleave = function(){ SChatBox.OnImageHover('%s', false) };",
			link
		)
	end
...
function JSBuilder:CreateCode(code, font, inline)
	...
	AddLine(
		lines,
		"elCode.style.backgroundColor = '%s';",
		ColorToJs(JSBuilder.codeBackgroundColor)
	)
...
function JSBuilder:CreateEmbed(url)
	...
	AddLine(
		lines,
		"elEmbed.onclick = function(){ SChatBox.OnClickLink('%s') };",
		url
	)
...
function JSBuilder:OnHTTPResponse(embedId, body, url)
	...
	if props["site_name"] then
		...
		AddLine(
			lines,
			"elName.textContent = '%s';",
			SafeString(props["site_name"])
		)
	end
...
local XML_PROFILE_START =
	'<?xml version="1.0" encoding="UTF-8" standalone="yes"?>\n<profile>'

local function ExtractAvatarFromXML(data)
	if string.StartsWith(data, XML_PROFILE_START) then
		return
	end

	local urlPattern = "<!%[CDATA%[(https://[%g%.]+/[%g]+%.jpg)%]%]>"
	local _, _, url =
		string.find(data, "<avatarMedium>" .. urlPattern .. "</avatarMedium>")

	if not url then
		_, _, url =
			string.find(data, "<avatarIcon>" .. urlPattern .. "</avatarIcon>")
	end
...
local function UpdateAllAvatars(steamId64, url)
	local code = ([[var avatarElements = document.getElementsByClassName('ply-%s');
...
function JSBuilder:FetchUserAvatarURL(id)
	...
	HTTP({
		url = string.format(
			"https://steamcommunity.com/profiles/%s?xml=true",
			id
		),
		method = "GET",
