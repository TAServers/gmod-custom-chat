...
hook.Add(
	"OnPlayerChat",
	"SChat.AddCustomTags",
	function(ply, text, isTeam, isDead)
		if not SChat.USE_TAGS then
			return
		end
...
hook.Add("player_connect_client", "schat_PlayerConnect", function(data)
	if not Tags.connection.showConnect then
		return
	end
	...
	SChat:AppendMessage({
		Color(255, 255, 255),
		Tags.connection.joinPrefix,
		Color(c[1], c[2], c[3]),
		name,
		Color(150, 150, 150),
		" <" .. data.networkid .. "> ",
		Color(255, 255, 255),
		Tags.connection.joinSuffix,
	})
end, HOOK_LOW)

hook.Add("player_disconnect", "schat_PlayerDisconnect", function(data)
	if not Tags.connection.showDisconnect then
		return
	end
	...
	SChat:AppendMessage({
		Color(255, 255, 255),
		Tags.connection.leavePrefix,
		Color(c[1], c[2], c[3]),
		name,
		Color(150, 150, 150),
		" <" .. data.networkid .. "> ",
		Color(255, 255, 255),
		Tags.connection.leaveSuffix,
		Color(150, 150, 150),
		" (" .. data.reason .. ")",
	})
end, HOOK_LOW)
...
function Tags:ShowChatTagsPanel()
	...
	btnAddUser.DoClick = function()
		...
		if byId[id] then
			Derma_Message(
				"Thah SteamID is on the list already!",
				"Input error",
				"OK"
			)
	...
	btnApply.DoClick = function()
		...
		Derma_Query(
			"This action will apply the chat tags on this server.\nAre you sure?",
			"Set Chat Tags",
			"Yes",
			function()
